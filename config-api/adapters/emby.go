...
// Emby adapter — requires API key (Dashboard → API Keys).
// adapter_config: { token: "${EMBY_TOKEN}" }
func fetchEmbyStats(cfg map[string]interface{}, baseURL string) AdapterResult {
	token := cfgStr(cfg, "token")
	if token == "" {
		return errResult("emby", "token not configured")
	}
	...
	if err := do("/System/Info", &info); err != nil {
		return errResult("emby", "API unreachable: "+err.Error())
	}
	if info.Version != "" {
	...
	if err := do("/Sessions", &sessions); err == nil {
		...
		stats = append(stats, StatItem{Label: "Streams", Value: fmt.Sprintf("%d active", playing), Status: st})
	}
	var counts struct {
	...
	return AdapterResult{Adapter: "emby", Ok: true, Stats: stats}
}
